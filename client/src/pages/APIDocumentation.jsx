...
export default function APIDocumentation() {
    ...
    useEffect(() => {
        if (mobileMenuOpen) {
            ...
            const menuButton = buttons.find(btn => btn.textContent.includes('Table of Contents'));
            if (menuButton) {
                const container = menuButton.closest('div');
                if (container) {
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            headings.forEach((heading) => {
                ...
                // If heading is near the top of the viewport (top priority)
                if (rect.top <= 150 && rect.top >= 0) {
                    currentSection = heading.id;
                }

                // Track the closest heading to the top for fallback
            ...
            // If no section found near top, use the closest one
    ...
    // Scroll table of contents to keep active section visible at center
    useEffect(() => {
        if (activeSection) {
    ...
    const fetchDocumentation = async () => {
        try {
            setLoading(true);
            setError(null);
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/api-documentation`);
            ...
            processedContent = processedContent.replace(/^#\s+SourceToLive Backend API Documentation\s*\n+/m, '');
    ...
    return (
        <div className="w-full max-w-7xl mx-auto px-4 py-8 lg:py-12">
            ...
            <div className="mb-12 relative">
                ...
                <div className="absolute inset-0 bg-linear-to-r from-[#3B7DC3]/10 to-[#4CAF50]/10 rounded-2xl blur-2xl -z-10 opacity-60"></div>

                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-6 bg-linear-to-br from-white/80 to-white/50 backdrop-blur-md border border-white/60 rounded-2xl p-6 shadow-[0_4px_20px_rgba(59,125,195,0.1)]">
                    <div className="p-4 rounded-xl bg-linear-to-br from-[#3B7DC3] to-[#2A5F99] shadow-lg">
                        <BookOpen className="h-8 w-8 text-white" />
                    </div>
                    <div className="flex-1">
                        <div className="inline-flex items-center gap-2 mb-2 px-3 py-1 rounded-full bg-[#3B7DC3]/10 border border-[#3B7DC3]/30">
                            <span className="w-2 h-2 rounded-full bg-[#3B7DC3]"></span>
                            <span className="text-xs font-semibold text-[#3B7DC3] uppercase tracking-wide">API Reference</span>
                        </div>
                        <h1 className="text-2xl sm:text-4xl lg:text-5xl font-bold text-gray-900 tracking-tight mb-2">
                            SourceToLive Backend
                            <span className="block bg-linear-to-r from-[#3B7DC3] to-[#4CAF50] bg-clip-text text-transparent">
                                API Documentation
                            </span>
                        </h1>
                        <p className="text-gray-600 text-sm sm:text-base max-w-2xl">
                            Complete API reference guide for SourceToLive deployment platform. Learn how to integrate, authenticate, and manage projects programmatically.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
                ...
                <div className="lg:hidden mb-4 sticky top-14 z-40 bg-white ">
                    <button
                        ...
                        className="cursor-pointer w-full flex items-center justify-between px-6 py-4 rounded-xl border border-white/60 bg-linear-to-r from-[#3B7DC3]/10 to-[#4CAF50]/10 backdrop-blur-md shadow-[0_4px_16px_rgba(15,23,42,0.08)] hover:bg-linear-to-r hover:from-[#3B7DC3]/20 hover:to-[#4CAF50]/20 transition-all border-b"
                    >
                        <div className="flex items-center gap-2">
                            <BookOpen className="h-5 w-5 text-[#3B7DC3]" />
                            <span className="font-bold text-gray-900 uppercase tracking-wider text-sm">Table of Contents</span>
                        </div>
                        {mobileMenuOpen ? (
                            <X className="h-5 w-5 text-[#3B7DC3]" />
                        ) : (
                            <Menu className="h-5 w-5 text-[#3B7DC3]" />
                        )}
                    </button>
                    ...
                    {mobileMenuOpen && (
                        <div className="sticky top-24 mt-2 rounded-xl border border-white/60 bg-white/70 backdrop-blur-md shadow-[0_4px_16px_rgba(15,23,42,0.08)] animate-in fade-in slide-in-from-top-2 duration-200 max-h-[calc(100vh-200px)] overflow-y-auto">
                                    ...
                                    tableOfContents.map((item, idx) => (
                                        <a
                                            ...
                                            className={`group relative flex items-start gap-3 px-4 py-2.5 rounded-lg transition-all duration-200 ease-in-out text-sm ${activeSection === item.id
                                                ? 'bg-[#3B7DC3] text-white shadow-md shadow-[#3B7DC3]/20'
                                                : 'text-gray-700 hover:text-[#3B7DC3] hover:bg-[#3B7DC3]/5'
                                                }`}
                                        >
                                            <span className={`shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-200 ease-in-out ${activeSection === item.id
                                                ...
                                                : 'bg-[#3B7DC3]/20 text-[#3B7DC3] group-hover:bg-[#3B7DC3]/40'
                                                }`}>
                                                {idx + 1}
                                            </span>
                            ...
                            <div className="px-4 py-3 border-t border-white/60 bg-linear-to-r from-[#3B7DC3]/5 to-[#4CAF50]/5 text-xs text-gray-600 text-center">
                ...
                <aside className="hidden lg:block lg:col-span-1">
                    <div className="sticky top-20 rounded-xl border border-white/60 bg-white/70 backdrop-blur-md shadow-[0_4px_16px_rgba(15,23,42,0.08)] overflow-hidden">
                        {/* Header */}
                        <div className="bg-linear-to-r from-[#3B7DC3]/10 to-[#4CAF50]/10 px-6 py-4 border-b border-white/60">
                        ...
                        <div className="flex items-center justify-center py-24">
                            <div className="flex flex-col items-center gap-4">
                                <div className="relative">
                                    <Loader className="h-10 w-10 animate-spin text-[#3B7DC3]" />
                                    <div className="absolute inset-0 bg-[#3B7DC3]/20 rounded-full blur-lg -z-10"></div>
                                </div>
                            ...
                            <ReactMarkdown
                                ...
                                components={{
                                    h1: ({ node, children, ...props }) => (
                                        <h1 className="text-2xl sm:text-4xl font-bold mt-12 mb-6 text-gray-900 bg-linear-to-r from-[#3B7DC3] to-[#2A5F99] bg-clip-text pb-2" {...props}>
                                            {children}
                                        </h1>
                                    ),
                                        ...
                                        return (
                                            <div className="mt-10 mb-6">
                                                <h2 id={id} className="text-3xl sm:text-4xl font-bold text-gray-900 mb-3 flex items-center gap-3 scroll-mt-24 group" {...props}>
                                                    <span className="inline-block w-1 h-10 bg-linear-to-b from-[#3B7DC3] to-[#4CAF50] rounded-full"></span>
                                                    {children}
                                                </h2>
                                                <div className="h-0.5 w-24 bg-linear-to-r from-[#3B7DC3] to-transparent"></div>
                                            </div>
                                        );
                                    },
                                    h3: ({ node, children, ...props }) => (
                                        <h3 className="text-2xl font-bold mt-8 mb-4 text-gray-900 flex items-center gap-2 pt-2" {...props}>
                                            <span className="inline-block w-1.5 h-6 bg-[#3B7DC3] rounded-full"></span>
                                            {children}
                                        </h3>
                                    ),
                                    ...
                                    li: ({ node, children, ...props }) => (
                                        <li className="text-gray-700 text-sm sm:text-base flex gap-3 items-start" {...props}>
                                            <span className="shrink-0 inline-flex items-center justify-center w-6 h-6 rounded-full bg-[#3B7DC3]/10 text-[#3B7DC3] font-semibold text-xs mt-0.5">•</span>
                                            <span className="flex-1 pt-0.5">{children}</span>
                                        </li>
                                    ),
                                    code: ({ node, inline, children, ...props }) =>
                                        inline ? (
                                            <code className="bg-gray-100 text-[#2A5F99] px-1.5 py-0.5 rounded font-mono text-xs whitespace-nowrap" {...props}>
                                                {children}
                                            </code>
                                        ) : (
                                    ...
                                    th: ({ node, children, ...props }) => (
                                        <th className="bg-linear-to-r from-[#3B7DC3] to-[#2A5F99] font-bold py-3 px-4 text-left text-white text-xs sm:text-sm border border-gray-300 first:rounded-tl-lg last:rounded-tr-lg" {...props}>
                                            {children}
                                        </th>
                                    ),
                                    ...
                                    a: ({ node, children, ...props }) => (
                                        <a className="text-[#3B7DC3] hover:text-[#2A5F99] font-medium transition-colors hover:underline" target="_blank" rel="noopener noreferrer" {...props}>
                                            {children}
                                        </a>
                                    ),
                                    blockquote: ({ node, children, ...props }) => (
                                        <blockquote className="pl-4 border-l-4 border-[#3B7DC3] text-gray-600 italic my-4 bg-blue-50/50 py-2 pr-4 rounded-r" {...props}>
                                            {children}
                                        </blockquote>
                                    ),
                                    hr: ({ node, ...props }) => (
